...
| File | Role |
|------|------|
| `pythoncompute_addin.*` | `com.sun.star.sheet.AddIn` — `org.collaboraoffice.sheet.addin.PythonComputeFunctions` (`getPy` / `getPython`) |
| `pythoncompute_volatile.*` | `XVolatileResult` holder |
| `pythoncompute_anyjson.*` | Calc `Any` ↔ dumb JSON via `tools::JsonWriter` (emit) + a small typed hand parser; no NumPy |
| `pythoncompute_bridge.*` / `.h` | Pending map + param→volatile cache + C API (`pythoncompute_set_emitter`, `pythoncompute_complete_json`) |
...
**Solar:** `finish()` always notifies under `SolarMutexGuard`. Kit Unipoll delivers
`pythoncomputeresult:` inside `kitPoll` with Solar released; re-acquiring lets
`ScAddInListener::modified` → `TrackFormulas` invalidate tiles (avoids sticky `#BUSY!`).
